...
private:
    ...
    // Display
    bool displayIsOn_ = true;
    bool blinkRunning_ = false;
    unsigned long displayBlinkMillis_ = 0;
    bool crawlRunning_ = false;
    unsigned long crawlMillis_ = 0;
    uint16_t currCrawlStep_ = 0;
    bool showEnding_ = false;

    uint16_t notes_ = 0;
    uint16_t currNote_ = 0;
    unsigned long audioStartMillis_ = 0;

    unsigned int freq_[NOTES] {};
    unsigned long durations_[NOTES] {};
    // We also have a pause at the end
    unsigned long delays_[NOTES + 1] {};
};
